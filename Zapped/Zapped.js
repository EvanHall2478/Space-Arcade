...
//Every certain interval of time have a chace of spawning a heart somewher inside of the bounderies 
/* function extraLives(){
...
function drawLives() {
	allHearts.forEach(function (heart) {
		// draw your heart
		ctx.beginPath();
		ctx.arc(heart.x, heart.y, heart.r, 2 * Math.PI, false); // circle
		ctx.fillStyle = 'red'; 
		ctx.fill();
	})
}
//check the time interval for the chance of spawning in a heart
setInterval(function() {
	allHearts.length()+1;
}, 5000);
*/ 
...
function gameOver() { 
	if (Lives == 0) { 
		...
		setTimeout(function() {
			location.href = '/~/Space_Arcade/Game_Menu'
		}, 2000)
		return true;
	} 
	return false; 
